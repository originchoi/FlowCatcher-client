...
아래의 다이어그램은 D3.js 라이브러리를 사용하여 웹 브라우저 내에서 데이터 시각화를 구현하는 과정을 나타냅니다. 

<p align="center">
  <img width="700" alt="image" src="https://github.com/originchoi/FlowCatcher-client/assets/116258834/3a3e70cb-9de0-4f4c-9bd7-56f384b46289">
</p>

각 단계별로 일어나는 주요 활동과 상호작용을 자세히 설명하겠습니다.

- 브라우저 (클라이언트) → D3.js 라이브러리
  - 데이터 시각화 초기화: 사용자(브라우저)가 데이터 시각화를 위한 요청을 시작합니다. 이는 D3.js 라이브러리에 데이터와 시각화에 필요한 설정(예: 차트 유형, 데이터 포인트 등)을 전달함으로써 수행됩니다.

- D3.js 라이브러리 → SVG 요소
  - 노드 생성/업데이트: D3.js는 전달받은 데이터를 기반으로 그래프의 노드(점)를 생성하거나 업데이트합니다. 노드는 데이터 포인트를 시각적으로 표현하며, 각 노드는 특정 데이터 값과 연결됩니다.
  - 링크 생성/업데이트: 노드 간의 연결선(링크)도 마찬가지로 생성되거나 업데이트됩니다. 이 링크는 데이터 포인트들 간의 관계나 흐름을 나타냅니다.
  - 줌 및 드래그 행동 적용: 사용자가 차트를 확대하거나 이동할 수 있도록 줌 및 드래그 기능을 구현합니다. 이는 보다 세밀한 데이터 검토를 가능하게 하며 사용자 상호작용을 향상시킵니다.

- SVG 요소 동적 업데이트
  - 동적 반응: 위에서 언급한 노드와 링크의 생성 및 업데이트는 SVG 요소 내에서 동적으로 이루어집니다. SVG는 Scalable Vector Graphics의 약자로, XML 기반의 벡터 이미지 포맷입니다. D3.js는 이 SVG를 사용하여 데이터를 그래픽으로 변환하고, 사용자의 상호작용에 따라 그래픽을 동적으로 조정합니다.

</br>

#### 1. 세션 데이터 처리 및 그래프 데이터 준비

이 과정은 사용자 세션 데이터를 분석하여 페이지 노드와 연결 링크를 생성합니다. 각 페이지 방문은 노드로 표현되며, 사용자의 페이지 이동 경로는 링크로 연결됩니다.

<details>
  <summary><strong>세션 데이터 처리 및 그래프 데이터 준비 코드 보기</strong></summary>

```jsx
function processPageViewData(sessions) {
  const nodes = [];  // 그래프의 노드를 저장할 배열
  const links = [];  // 그래프의 링크를 저장할 배열
  const linkCounts = {};  // 링크의 발생 횟수를 카운팅
  const visitCounts = {};  // 각 URL 방문 횟수를 카운팅
  const exitCounts = {};
...
**주요 내용:**
- 노드 처리: 각 세션과 페이지뷰를 반복하면서, 방문한 URL을 기반으로 노드를 생성하고 업데이트합니다. 각 노드는 페이지의 URL, 제목, 레퍼러, 타임스탬프 등의 정보를 포함합니다.
- 링크 처리: 연속된 페이지 방문 사이에 링크를 생성하고, 이 링크는 페이지 간의 이동을 나타냅니다. 자기 자신으로의 링크(자기 루프)는 페이지가 새로고침된 경우를 표시합니다.
- 페이지 방문 수 계산: 각 페이지 URL의 방문 횟수를 집계합니다.

이 과정을 통해 각 세션에서 사용자의 페이지 방문 경로를 그래픽 요소로 변환할 수 있는 데이터 구조를 완성합니다. 생성된 노드와 링크는 추후 D3.js를 사용하여 시각적인 그래프로 표현됩니다.

이렇게 세션 데이터를 통해 사용자의 웹사이트 탐색 패턴을 분석하고 이를 그래프로 시각화함으로써, 사용자 경험 개선 및 웹사이트 최적화에 필요한 인사이트를 제공할 수 있습니다. 이 함수는 효과적인 데이터 시각화를 위한 기반 데이터를 준비하는 중요한 단계입니다.
...
이 과정에서는 준비된 노드와 링크 데이터를 사용하여 실제 SVG 요소로 그래프를 시각화합니다. D3의 드래그 및 줌 기능을 활용하여 인터랙티브한 사용자 경험을 제공합니다.

<details>
  <summary><strong>Force-Directed Graph 시각화 코드 보기</strong></summary>
...
**주요 내용:**
- SVG 초기화: 함수 시작 시, D3를 사용하여 선택된 SVG 요소 내의 모든 내용을 초기화합니다. 이는 이전 그래프의 잔재를 제거하고 새 그래프를 깨끗한 상태에서 시작할 수 있도록 합니다.
- 줌 기능 설정: 사용자가 그래프의 특정 부분을 확대하거나 축소할 수 있도록 줌 기능을 설정합니다. 이는 d3.zoom() 메소드를 사용하여 구현되며, 사용자의 휠 동작이나 터치 제스처에 반응하여 그래프의 스케일과 위치를 동적으로 조정합니다.
- 노드 시각화: 각 노드는 사각형 형태로 표현되며, 페이지의 메타데이터(예: URL, 제목)를 시각적으로 나타냅니다. 노드는 드래그 가능하도록 설정되어 있어, 사용자가 마우스 또는 터치를 통해 그래프 상에서 자유롭게 위치를 조정할 수 있습니다. 이는 d3.drag() 메소드를 활용하여 구현됩니다.
- 링크 시각화: 링크는 페이지 간의 이동 경로를 나타내며, path SVG 요소로 표현됩니다. 각 링크는 시작 페이지와 목표 페이지 사이를 연결하며, 자기 자신으로의 링크(예: 페이지 새로고침)는 특별한 색상과 마커로 구분됩니다. 링크의 스타일과 마커는 CSS 스타일과 함께 D3의 데이터 바인딩 기능을 사용하여 설정됩니다.

이 시각화 과정은 사용자에게 웹 사이트 내의 탐색 패턴과 페이지 간의 연결 구조를 직관적으로 이해할 수 있는 수단을 제공합니다. 인터랙티브한 기능은 사용자가 그래프를 직접 조작하면서 다양한 관점에서 데이터를 탐색할 수 있게 돕습니다.
